...
def runjob(script, lines):
    cmd = 'perl /rhome/cjinfeng/software/bin/qsub-pbs.pl --maxjob 40 --lines %s --interval 120 --resource nodes=1:ppn=4,walltime=100:00:00,mem=10G --convert no %s' %(lines, script)
    #print cmd 
    os.system(cmd)
...
def main():
    ...
    cmp_files = glob.glob('%s/*.sort.cmp.h5' %(args.cmpdir))
    cmd_lines = []
    for h5 in sorted(cmp_files):
        prefix = os.path.split(h5)[1]
        prefix = re.sub(r'\.sort\.cmp\.h5', r'', prefix)
        prefix = '%s/%s' %(args.project, prefix)
        cmd = 'quiver -j4 %s -p C2.NoQVsModel -r %s -o %s.gff -o %s.consensus.fasta -o %s.consensus.fastq' %(os.path.abspath(h5), os.path.abspath(args.ref), os.path.abspath(prefix), os.path.abspath(prefix), os.path.abspath(prefix))
        #print cmd
