...
my ($Queue, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub,$Resource,$Job_prefix,$Task,$Mem,$Time,$Verbose, $Help);
GetOptions(
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
	"interval:i"=>\$Interval,
...
while(<IN>){
	...
	if ($line_mark % $Lines == 0) {
		open OUT,">$Work_dir/$Job_prefix\_$Job_mark.sh" || die "failed creat $Job_prefix\_$Job_mark.sh";
                print OUT "#!/bin/bash\n\nPYTHONPATH=/rhome/cjinfeng/BigData/00.RD/Assembly/Pacbio/install/pythonlib/:/rhome/cjinfeng/BigData/00.RD/Assembly/Pacbio/install/pythonlib/lib64/python2.7/site-packages/:/rhome/cjinfeng/BigData/00.RD/Assembly/Pacbio/install/pythonlib/lib/python2.7/site-packages/:\$PYTHONPATH\n\n";
...
while (@Shell) {
	...
	my $job_cmd = "sbatch --workdir=$Work_dir ";  ## -l h_vmem=16G,s_core=8 
	$job_cmd .= "-p $Queue "  if(defined $Queue); ##set queue
        $job_cmd .= "-t $Time " if(defined $Time); ##set resource
        $job_cmd .= "--mem=$Mem " if(defined $Mem); ##set resource
        $job_cmd .= "--ntasks=$Task " if(defined $Task); ##set resource
	##warn $job_cmd;
	for (my $i=0; $i<@Shell; $i++) {
		while (1) {
			my $run_num = run_count(\%Alljob,\%Runjob);
			if ($i < $Maxjob || ($run_num != -1 && $run_num < $Maxjob) ) {
				my $job_return = `$job_cmd -o $Shell[$i].o -e $Shell[$i].e $Shell[$i]`;
                                print "$job_cmd -o $Shell[$i].o -e $Shell[$i].e $Shell[$i]\n";
                                print "$job_return\n";
				my $job_id = $1 if($job_return =~ /(\d+)/); #300021.torque01.int.bioinfo.ucr.edu
	...
	foreach my $job_id (sort keys %Alljob) {
		...
		if (-f "$shell_file.o") {
			...
			#$content = join("",<IN>);
			#close IN;
			$content = `tail -n 1000 $shell_file.o`;
		}
		##check whether the job has been killed during running time
		if ($content !~ /This-Work-is-Completed!/) {
			$Error{$job_id} = $shell_file;
			print OUT "In qsub cycle $qsub_cycle, In $shell_file.o,  \"This-Work-is-Completed!\" is not found, so this work may be unfinished\n";
		}
		...
		if (-f "$shell_file.e") {
			...
			#$content = join("",<IN>);
			#close IN;
			$content = `tail  -n 1000 $shell_file.e`;
		}
