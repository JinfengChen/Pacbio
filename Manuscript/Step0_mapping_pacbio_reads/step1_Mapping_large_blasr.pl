#!/usr/bin/perl
=header
Map Pacbio long reads or contig to genome using blasr.
USE fullpath of files.
--ref:   reference sequence
--1:     Pacbio long reads or contig from assembly
--tool:  mapping tools: blasr
...
my %opt;
GetOptions(\%opt,"ref:s","1:s","2:s","tool:s","cpu:s","bam","verbose","output:s", "split:s", "project:s", "genomealign","help");
...
if (exists $opt{1}){
   if ($opt{tool}=~/bwa/){
      ...
      my @split;
      if ($opt{1} =~ /\.f.*a$/){
          push @split, "perl $fasplit -cuts $opt{split} $opt{1} --outdir $opt{output}";
      }else{
          push @split, "perl $fqsplit -s $opt{split} -o $opt{output} $opt{1}";
      }
      my $cmd=join("\n",@split);
      ...
      `perl /rhome/cjinfeng/BigData/software/bin/qsub-slurm.pl --lines 1 --interval 120 --task 1 --mem 5G --time 100:00:00 --convert no $opt{project}.split.sh`;

      ### map small files
      my @map;
      my @fqs= $opt{1} =~ /\.f.*a$/ ? sort glob("$opt{output}/*.f*a") :  sort glob("$opt{output}/*.f*q");
      ...
      if ($opt{split} > 1000){
          `perl /rhome/cjinfeng/BigData/software/bin/qsub-slurm.pl --maxjob 60 --lines 2 --interval 120 --task 1 --mem 15G --time 100:00:00 --convert no $opt{project}.map.sh`;
      }else{
          `perl /rhome/cjinfeng/BigData/software/bin/qsub-slurm.pl --maxjob 60 --lines 4 --interval 120 --task 1 --mem 15G --time 100:00:00 --convert no $opt{project}.map.sh`;
      }

      ### merge and clean tmp files
      my @merge;
      unless (-e "$opt{output}.sam"){
          my @tempsams = glob("$opt{output}/*.sam");
          push (@merge, "grep \"^@\" $tempsams[0] > $opt{output}.sam");
      ...
      `perl /rhome/cjinfeng/BigData/software/bin/qsub-slurm.pl --lines 5 --interval 120 --task 1 --mem 20G --time 100:00:00 --convert no $opt{project}.merge.sh`;
      ...
      unless ($opt{verbose}){
          `perl /rhome/cjinfeng/BigData/software/bin/qsub-slurm.pl --lines 3 --interval 120 --task 1 --mem 10G --time 10:00:00 --convert no $opt{project}.clear.sh`;
      }
=cut
      print "Align Read 1!\n";
