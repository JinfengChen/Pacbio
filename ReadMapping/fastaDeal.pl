...
sub cut_fasta_advanced{
	...
	##make the last file
	if ($Cur_content) {
		open OUT,">$out_dir/p$file_mark.$file_name" || die "fail";
		print OUT $Cur_content;
		close OUT;
	}
...
sub cut_fasta{
	...
	while (<IN>) {
		...
		if($num_loop % $seq_num == 0){
			print STDERR "generating:  $out_dir/p$mark.$file_name\n" if( $Verbose && $dir_rank == 1);
			open ( OUT,">$out_dir/p$mark.$file_name" ) if($dir_rank == 1);

			print STDERR "generating:  $out_dir/$submark/p$mark.$file_name\n" if($Verbose && $dir_rank == 2);
			open ( OUT,">$out_dir/$submark/p$mark.$file_name" ) if($dir_rank == 2);
